...
class OCRPDFWorker(QThread):
    ...
    def run(self):
        ...
        try:
            ...
            # Você pode substituir esse trecho por etapas específicas, se houver
            # no seu caso, não existe, então ele apenas simulará o progresso
            ocrmypdf.ocr(self.input_pdf, self.output_pdf, lang='por')
            self.progress.emit(90) # Simula fim do processo de OCR
            time.sleep(0.5)
            self.progress.emit(100)
            self.result.emit(f"{COMPLETED_TEXT} Arquivo salvo em: {self.output_pdf}")
...
class OCRPDFApp(QMainWindow):
    ...
    def select_pdf(self):
        ...
        if file_path:
            ...
            self.select_output_button.setEnabled(True) # Habilita o botão de saída
            self.start_button.setEnabled(False) #Desabilita o botão de iniciar até definir o local de saída
    ...
    def select_output(self):
      ...
      if self.input_pdf:
            ...
            self.output_pdf, _ = QFileDialog.getSaveFileName(
                self, "Salvar PDF OCR", base_name + "_ocr.pdf", "PDF Files (*.pdf)"
            )
            if self.output_pdf:
               self.label.setText(f"PDF selecionado: {self.input_pdf}\nArquivo de saída: {self.output_pdf}")
    ...
    def start_ocr(self):
        ...
        if self.input_pdf and self.output_pdf:
            try:
                ...
                QApplication.setOverrideCursor(QCursor(Qt.CursorShape.WaitCursor))  # Coloca cursor de espera
    ...
    def display_result(self, result):
        ...
        reply = QMessageBox.question(
            self, "Conclusão", "OCR concluído. Deseja Continuar ?",
            QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No,
            QMessageBox.StandardButton.No,
        )
        
        if reply == QMessageBox.StandardButton.No: # Tratamento da resposta "Não"
    ...
    def _validate_output_path(self):
        ...
        if os.path.exists(self.output_pdf):
            try:
                open(self.output_pdf, "a").close()  # Tenta abrir o arquivo para escrita
            except IOError:
                raise Exception("O arquivo de saída já existe e não pode ser acessado para escrita.")
        else:
            try:
