...
class Worker(QThread):
    ...
    def extrair_nomes_e_valores_pagos(
        self, pdf_path, total_pages, current_stage, total_stages
    ):
    ...
    def extract_data(self, pdf_path, total_pages, current_stage, total_stages):
        ...
        try:
            ...
            for page_num, page in enumerate(reader.pages, start=1):
                ...
                progress = ((current_stage - 1) / total_stages) + (1 / total_stages) * (
                    page_num / total_pages
                )
                self.progress_update.emit(int(progress * 100))
    ...
    def criar_pasta_saida(self, pdf_path, base_output_dir):
        ...
        try:
            ...
            nome_pasta = nome_arquivo.split(" - ")[
                0
            ].strip()  # Pega o nome antes do " - "
    ...
    def comparar_txts_busca_parcial(
        self,
        arquivo1_path,
        arquivo2_path,
        arquivo_saida_path,
        ...
        try:
            with open(arquivo1_path, "r", encoding="utf-8") as arquivo1, open(
                arquivo2_path, "r", encoding="utf-8"
            ) as arquivo2, open(
                arquivo_saida_path, "w", encoding="utf-8"
            ) as arquivo_saida:
    ...
    def comparar_txts_com_busca_parcial(
        self,
        arquivo1_path,
        arquivo2_path,
        arquivo_saida_exato,
        arquivo_saida_diferente,
        arquivo_nomes_nao_encontrados,
        total_lines,
        current_stage,
        total_stages,
    ):
        ...
        try:
            ...
            with open(arquivo2_path, "r", encoding="utf-8") as arquivo2, open(
                arquivo_saida_exato, "w", encoding="utf-8"
            ) as arquivo_saida_exato:
                for line_num, linha in enumerate(arquivo2, start=1):
                    match = re.match(r"(.+?):\s*([\d,.]+)", linha)
                    if match:
                        nome2_completo = match.group(1).strip()
                        ...
                        if (
                            nome2_completo in dados_arquivo1
                            and dados_arquivo1[nome2_completo] == valor2
                        ):
            ...
            with open(
                arquivo_nomes_nao_encontrados, "w", encoding="utf-8"
            ) as arquivo_nomes_nao_encontrados_file:
                with open(
                    arquivo_saida_diferente, "w", encoding="utf-8"
                ) as arquivo_saida_diferente:
                    for nome_original, valor_original in tqdm(
                        nomes_nao_encontrados, desc="Comparando arquivos (Parcial)"
                    ):
                        ...
                        if partes_nome:
                            ...
                            for i in range(1, len(partes_nome) + 1):
                                ...
                                if i == 1 and len(partes_nome) > 1:
                                    nome_parcial = partes_nome[
                                        0
                                    ]  # Pega o primeiro nome
                                    nome_parcial += (
                                        " " + partes_nome[1][0]
                                    )  # Adiciona a primeira letra do segundo nome

                                correspondencias = [
                                    nome
                                    for nome, valor in dados_arquivo1.items()
                                    if nome.startswith(nome_parcial)
                                    and valor == valor_original
                                ]
                                ...
                                if len(correspondencias) == 1:
                                    ...
                                    correspondencias_diferentes.append(
                                        (nome_original, nome_encontrado, valor_original)
                                    )
                                    encontrado = True
                                    break
                        if not encontrado:
                            ...
                            arquivo_nomes_nao_encontrados_file.write(
                                f"{nome_original}: {valor_original}\n"
                ...
                with open(arquivo_saida_path, "a", encoding="utf-8") as arquivo_saida:
                    for line_num, linha in enumerate(arquivo_entrada, start=1):
                        if "Encontrado como" in linha:
                            match = re.match(
                                r"Encontrado como:\s*(.*?):\s*([\d,.]+)", linha.strip()
                            )
                            if match:
                                nome = match.group(1).strip()
                                valor = match.group(2).strip()
    ...
    def destacar_nomes_no_pdf(
        self,
        pdf_path,
        txt_path,
        output_pdf_path,
        nomes_nao_encontrados_path,
        total_pages,
        current_stage,
        total_stages,
    ):
        ...
        if nomes_nao_destacados:
            ...
            try:
                nomes_nao_encontrados_path = os.path.join(
                    os.path.dirname(output_pdf_path), "nomes_nao_destacados.txt"
                )  # Caminho absoluto
                ...
                print(
                    f"Nomes não destacados: {nomes_nao_destacados}"
                )  # Imprimir nomes não destacados em caso de erro
    ...
    def salvar_arquivos_na_pasta(
        ...
        if pasta_saida:
            ...
            try:
                ...
                novo_arquivo_saida_path = Path(
                    pasta_saida, Path(arquivo_saida_path).name
                )
                novo_arquivo_saida_exato = Path(
                    pasta_saida, Path(arquivo_saida_exato).name
                )
                novo_arquivo_saida_diferente = Path(
                    pasta_saida, Path(arquivo_saida_diferente).name
                )
                novo_output_pdf = Path(pasta_saida, Path(output_pdf).name)
                novo_nomes_nao_encontrados_path = Path(
                    pasta_saida, Path(nomes_nao_encontrados_path).name
                )
                novo_arquivo_nomes_nao_encontrados = Path(
                    pasta_saida, Path(arquivo_nomes_nao_encontrados).name
                )
                ...
                if os.path.exists(nomes_nao_encontrados_path):
                    shutil.copy2(
                        nomes_nao_encontrados_path, novo_nomes_nao_encontrados_path
                    )
                shutil.copy2(
                    arquivo_nomes_nao_encontrados, novo_arquivo_nomes_nao_encontrados
                )
                return pasta_saida
    ...
    def processar_pdfs(self, pdf_pagos_path, pdf_projetos_dir, base_output_dir):
        ...
        if not pdf_projetos_files:
            QMessageBox.critical(
                None, "Erro", "Não foram encontrados arquivos PDF na pasta selecionada."
            )
            return
        ...
        for pdf_todos in pdf_projetos_files:
            ...
            if not pasta_saida:
                QMessageBox.critical(
                    None,
                    "Erro",
                    f"Não foi possível criar a pasta de saída para '{Path(pdf_todos).name}'.",
                )
                continue
            ...
            arquivo_saida_diferente = str(
                Path(pasta_saida, "Funcionarios_nomes_diferente.txt")
            )
            output_pdf = str(Path(pasta_saida, f"{Path(pdf_todos).stem}_destacado.pdf"))
            nomes_nao_encontrados_path = str(
                Path(pasta_saida, "nomes_nao_destacados.txt")
            )
            arquivo_nomes_nao_encontrados = str(
                Path(pasta_saida, "Nomes não encontrados.txt")
            )
            ...
            separados = self.extrair_nomes_e_valores_pagos(
                pdf_pagos_path, total_pages_pagos, current_stage, total_stages
            )
            ...
            dados_projeto = self.extract_data(
                pdf_todos, total_pages_projetos, current_stage, total_stages
            )
            ...
            self.comparar_txts_busca_parcial(
                output_projeto,
                output_pagos,
                arquivo_saida_path,
                total_lines_projetos,
                current_stage,
                total_stages,
            )
            ...
            current_stage += 1
            self.comparar_txts_com_busca_parcial(
                arquivo1_path,
                arquivo2_path,
                arquivo_saida_exato,
                arquivo_saida_diferente,
            ...
            self.adicionar_nomes_encontrados(
                arquivo_saida_diferente,
                arquivo_saida_exato,
                total_lines_diferente,
                current_stage,
                total_stages,
            )
            ...
            self.destacar_nomes_no_pdf(
                pdf_pagos_path,
                arquivo_saida_exato,
                output_pdf,
                nomes_nao_encontrados_path,
                total_pages_destacados,
                current_stage,
                total_stages,
            )
            ...
            pasta_base = self.salvar_arquivos_na_pasta(
                pasta_saida,
                output_pagos,
                output_projeto,
                arquivo_saida_path,
...
class MainWindow(QMainWindow):
    ...
    def selecionar_arquivo_pagos(self):
        ...
        filename, _ = QFileDialog.getOpenFileName(
            self, "Selecione o PDF de Pagamentos", "", "Arquivos PDF (*.pdf)"
        )
        if filename:
    ...
    def selecionar_pasta_projetos(self):
        ...
        foldername = QFileDialog.getExistingDirectory(
            self, "Selecione a pasta com os PDFs de Projetos"
        )
        if foldername:
    ...
    def iniciar_processamento(self):
        ...
        if not pdf_pagos_path or not pdf_projetos_dir or not base_output_dir:
            QMessageBox.critical(
                self, "Erro", "Por favor, selecione todos os arquivos e pastas."
            )
            return
    ...
    def processamento_finalizado(self, base_output_dir):
        if (
            QMessageBox.question(
                self, "Concluído", "Todos os PDFs foram processados! Deseja continuar?"
            )
            == QMessageBox.Yes
        ):
            self.limpar_campos()
    ...
    def abrir_pasta(self, caminho_pasta):
        if sys.platform == "win32":  # Para Windows
            ...
            QMessageBox.critical(
                self, "Erro", f"Sistema Operacional não suportado: {sys.platform}"
            )
